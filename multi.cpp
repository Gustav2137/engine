...
public:
  ...
  uint64_t hash(uint64_t old_hash, Move m) {
    return board.Hash64Update(old_hash, m.value());
  }
  ...
  double heuristic() {
    ...
    cout << "material: " << material << "\nking_value: " << king_value
         ...
         << "\nspace: " << space << "\nactivity: " << activity << "\n";
#endif
    return 5.2 * material + 0.1 * king_value + 0.1 * mobility +
...
double heur_wrapper(Game &state, thc::Move m, bool check, bool mate,
  ...
  state.apply_move(m);
  uint64_t key = state.hash(Hash, m);
  auto remembered = TT.find(key);
  double h;
  if (remembered != TT.end()) {
    h = remembered->sc.value * (check ? 1000 : 1);
  } else {
    h = state.heuristic() * (check ? 1000 : 1);
  }
  state.undo_move(m);
  return h;
}
void wrapper(pair<Move, double> &out, Game &state, Move a, int depth,
             double alpha, double beta, bool maxi, uint64_t Hash);

pair<Move, double> alpha_beta(Game &state, Move a, int depth, double alpha,
                              double beta, bool maxi, uint64_t Hash) {
  auto lookup = TT.find(Hash);
  if (lookup != TT.end()) {
    if (lookup->sc.depth == depth) {
      return {lookup->sc.best, lookup->sc.value};
    }
  }
  if (depth == 0) {
  ...
  if (maxi) {
    ...
    if (moves_sorted.size()) {
      auto &[m, h] = moves_sorted[0];
      state.apply_move(m);
      pair<Move, double> new_m;
      auto first_search = thread(
          [&new_m, &state, alpha, beta, depth, m, H = state.hash(Hash, m)]() {
            new_m = alpha_beta(state, m, depth - 1, alpha, beta, false, H);
          });
      first_search.join();
      state.undo_move(m);
      if (new_m.sc > value) {
      ...
      vector<pair<Move, double>> answers(moves_sorted.size());
      vector<thread> threads(moves_sorted.size());
      for (int i = 1; i < moves_sorted.size(); i++) {
        auto &[m, h] = moves_sorted[i];
        Game ns = state;
        ns.apply_move(m);
        threads[i] = thread(wrapper, answers[i], ns, m, depth - 1, alpha, beta,
                            false, Hash);
      }
      for (int i = 1; i < moves_sorted.size(); i++) {
        threads[i].join();
        pair<Move,double> new_m = answers[i];
        if (new_m.sc > value) {
          value = new_m.sc;
          ans = m;
        }
  ...
  b:
    TT[Hash] = {value, cutoff ? Lower : Exact, depth, ans, 0};
#if debug
    ...
    for (auto &[m, h] : moves_sorted) {
      state.apply_move(m);
      pair<Move, double> new_m = alpha_beta(state, m, depth - 1, alpha, beta,
                                            true, state.hash(Hash, m));
      state.undo_move(m);
      if (new_m.sc < value) {
        value = new_m.sc;
        ans = m;
    ...
    cout << "Choosen: " << ans.value().NaturalOut(&state.board)
         ...
         << " with heuristic v of: " << value << "\n";
#endif
    TT[Hash] = {value, cutoff ? Upper : Exact, depth, ans, 0};
    if (depth != DEPTH and value > 0) {
      thc::DRAWTYPE res;
...
void wrapper(pair<Move, double> &out, Game &state, Move a, int depth,
             double alpha, double beta, bool maxi, uint64_t Hash) {
  out = alpha_beta(state, a, depth, alpha, beta, maxi, Hash);
}

Move run_alpha_beta(Game state) {
#if ERASE
  for (auto it = TT.begin(); it != TT.end();) {
    if (it->sc.old >= 2 * DEPTH) {
      it = TT.erase(it);
    } else {
      it->sc.old++;
    }
  ...
  auto ans = alpha_beta(state, {}, DEPTH, -INFINITY, INFINITY,
                        state.board.WhiteToPlay(), state.hash())
                 .fi;
#if T_MOVE
...
int main() {
  ...
  while (state.terminal() == thc::NOT_TERMINAL) {
    ...
    bool ok;
    thc::Move mv;
    if (choice == "b" and state.board.WhiteToPlay()) {
      choice.clear();
      goto black;
    }
    do {
